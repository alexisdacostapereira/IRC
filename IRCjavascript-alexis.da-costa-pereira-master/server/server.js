...
let Chat = require("./db/schema/chatSchema");
let User = require("./db/auth/userShema");
let Channel = require("./db/schema/channelSchema");
/*en test*/
...
io.on('connection', socket => {
    ...
    socket.on('chatMessage', msg => {
        ...
        console.log(msg)
        const cmdMsg = msg.split(' ');
        console.log(cmdMsg)
        if(cmdMsg[0]=="/delete"){
            if(cmdMsg[1]){
            deleteRoom= cmdMsg
            console.log(deleteRoom)
            Channel.deleteOne({ name: deleteRoom }, function (err) {
                if(err) console.log(err);
                console.log("Successful deletion");
            });
            io.to(user.room).emit('message', formatMessage(botName, msg+" Successful deletion"));
            }
            else{
                io.to(user.room).emit('message', formatMessage(botName, "erreur args to delete room"));
            }
        }
        else if(cmdMsg[0]=="/create"){
            if(cmdMsg[1]){
            createRoom= cmdMsg[1];
            console.log(createRoom);
            let chatChan = new Channel({ name: createRoom, createBy: user.username });
            chatChan.save();
            io.to(user.room).emit('message', formatMessage(botName, msg+" Successful created"));
            }
            else{
                io.to(user.room).emit('message', formatMessage(botName, "erreur args to create room"));
            }
        }
        else if(msg=="/users"){
            ...
            io.to(user.room).emit('leave', "leave");
        }
        else if(msg=="/list"){
            connect.then(db => {           
                Channel.find(function(err, result) {   
                    ...
                    result.forEach(element => {
                        myChans.push(element.name);
                    });
                console.log(myChans)
                io.to(user.room).emit('message', formatMessage(botName, myChans));
                });
            });
        } else if (msg=='/nick'){
            io.to(user.room).emit('name', "name");
        }else{
